...
const existingPlansCount = db.prepare('SELECT COUNT(*) as count FROM plans').get().count;
...
`);
  ...
  console.log('  ⏭️  Plans already exist, skipping seed (preserving admin changes)');
}

// Seed Offers (only if table is empty)
const existingOffersCount = db.prepare('SELECT COUNT(*) as count FROM offers').get().count;
...
`);
  ...
  console.log('  ⏭️  Offers already exist, skipping seed (preserving admin changes)');
}
